...
const welcome = require('./models/memberSchema');
const time = require('./helpers/timeFormat');
const ms = require('ms');
...
.on('messageCreate', async message => {
    ...
    if(commands) {
        ...
        if(commands.help.permission === '서버 관리하기' && !(message.member.permissions.has("MANAGE_GUILD"))) return message.channel.send('이 명령어를 사용하려면 **서버 관리하기** 권한이 필요해요.');
        else if(commands.help.permission === '메시지 관리하기' && !(message.member.permissions.has("MANAGE_MESSAGES"))) return message.channel.send('이 명령어를 사용하려면 **메시지 관리하기** 권한이 필요해요.');
        commands.run(client, message, args, prefix, time, embed);
    }
})
...
.on('guildMemberAdd', async member => {
    let config = await welcome.findOne({ guildId: member.guild.id });
    let msgData = config.welcomeMessage || `<@!${member.id}> 님! ${member.guild.name} 서버에 오신것을 환영합니다!`;
    ...
    if(config.welcomeControl === true) member.guild.channels.cache.get(config.welcomeChannelId).send(`${msg}`)
    else if(config.welcomeControl === false) return;
})
